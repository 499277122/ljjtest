...
(function($){
    ...
    //初始化表格
    $.fn.createEditTable = function(textAreaID){
        var tb = this;
        ...
        function startEdit(startEditValue){
            ...
            if(!$(this).is(".notChecked")){
                if($(this).is(".isFirstTd")){
                    ...
                    inp = $("<input type='number' class='form-control text-align-center'>");
                    inp.on("keyup",function(event){
                        var $inp = $(this);
                        //响应鼠标事件，允许左右方向键移动 
                        event = window.event || event;
                ...
                inp = $("<input type='hidden' class='form-control text-align-center'>");
            }
            $(this).html(inp);
        }
        //结束编辑
...
//                  $(arrtd[i]).html(arr[i]);
        ...
        function nt(event){
            if(event.ctrlKey && event.keyCode == 86){
                var ss = document.getElementsByClassName("tableCPtextArea")[0];
//              var beforeScrollY = $("#abc").scrollTop();
                $(ss).select();
//              setTimeout(function(){
//                  $("#abc").scrollTop(beforeScrollY);
//              },0)
